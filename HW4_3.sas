data cancer;
input ident PSA  volume weight age hyperplasia invasion capsular gleason;
logPSA = log(PSA);
datalines;
...
proc transreg data = cancer;
	model  boxcox(PSA)= identity(volume);
run;
