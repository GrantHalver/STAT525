data a1;
input id stay age infect culturing xray beds medschool region census nurse facilities;
datalines;
...
Set a1;
X1=0; X2=0; X3=0;
If region = 1 then X1=1;
If region= 2 then X2=1;
If region=3 then X3=1;

proc transreg data=a1;
	model boxcox(stay) = identity(infect) identity(age)
	identity(culturing) identity(xray) identity(beds)
	identity(medschool) identity(census) identity(nurse)
	identity(facilities) identity(X1) identity(X2)
	identity(X3);
run;
...
proc sgscatter;
	matrix stayinv infect age culturing xray beds census nurse facilities;
run;

proc corr;
var stayinv infect age culturing xray beds census nurse facilities;
run;

proc reg data=a1;
   model stayinv = infect age culturing xray beds medschool census nurse facilities X1 X2 X3/
   selection= adjrsq cp b aic sbc best=2;
run;

proc reg data = a1;
	model stayinv = infect age medschool census X1 X2 X3;
...
Data a4;
set a4;
yhat = 0.16976-0.00594*infect-0.00046675*age+0.00565*medschool-0.00001982*census-0.02611*X1-0.02003*X2-0.01722*X3;
err = stayinv-yhat;
errsq = err*err/22;

proc sql;
    select sum(errsq) as MSPR
    from a4;
quit;
